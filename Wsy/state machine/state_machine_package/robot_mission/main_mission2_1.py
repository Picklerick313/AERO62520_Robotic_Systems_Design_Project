...
class RobotStateMachine:
    def __init__(self, use_mock_modules: bool = False):
        ...
        self.mapping_duration = 10.0 # 5s for test
        ...
        # Return-to-start / bin state
        # NOTE:
        # The previous version used bin_pose=(0, 0). If (0, 0) is not a valid
        # reachable bin position in your map, Nav2 keeps trying until timeout.
        # Change these values to the real bin position in RViz/map coordinates.
        self.start_pose = Target(0.0, 0.0, 0.0)
        self.bin_pose = Target(1.0, 0.0, 0.0)

        # For state-machine demonstration/testing, set these True so the program
        # skips optional Nav2 navigation steps that are not ready yet.
        # For real robot/simulation navigation, set them False and tune the poses.
        self.skip_bin_navigation_for_test = True
        self.skip_start_navigation_for_test = True
    ...
    def start_exploring(self):
        ...
        if self.use_mock_modules:
            self.node.get_logger().info("Mock mode: skipping Nav2 exploration.")
    ...
    def navigate_to_bin_location(self):
        # Temporary demo/test path: avoid timeout when the real bin pose/Nav2 setup
        # is not ready yet. This allows the state machine to continue to PLACE_OBJECT.
        if self.skip_bin_navigation_for_test:
            self.node.get_logger().warn(
                "TEST MODE: skipping Nav2 navigation to bin. "
                "Set skip_bin_navigation_for_test=False for real navigation."
            )
            time.sleep(0.2)
            return True

        self.node.get_logger().info(
            f"Navigating to bin pose: x={self.bin_pose.x:.2f}, y={self.bin_pose.y:.2f}"
        )
    ...
    def go_back_to_starting_point(self):
        # Temporary demo/test path: avoid timeout when the real start pose/Nav2 setup
        # is not ready yet. This allows the mission to end cleanly after placing.
        if self.skip_start_navigation_for_test:
            self.node.get_logger().warn(
                "TEST MODE: skipping Nav2 navigation to starting point. "
                "Set skip_start_navigation_for_test=False for real navigation."
            )
            self.publish_stop()
            time.sleep(0.2)
            return True
        ...
        self.node.get_logger().info(
            f"Navigating back to starting pose: "
            f"x={self.start_pose.x:.2f}, y={self.start_pose.y:.2f}, yaw=0.00"
        )
    ...
    def navigate_to_fixed_pose(
        ...
        if not is_active:
            ...
            self.node.get_logger().info(
                f"Sending fixed-pose Nav2 goal: x={x:.2f}, y={y:.2f}, yaw={yaw:.2f}"
            )
    ...
    def fixed_pose_goal_response_callback(self, future, done_flag_name: str, success_flag_name: str):
        try:
            ...
            self.node.get_logger().error(f"Fixed-pose Nav2 goal request failed: {e}")
        ...
        self.node.get_logger().info("Fixed-pose Nav2 goal accepted.")
    ...
    def fixed_pose_result_callback(self, future, done_flag_name: str, success_flag_name: str):
        try:
            ...
            self.node.get_logger().error(f"Fixed-pose Nav2 result failed: {e}")
            setattr(self, success_flag_name, False)
            setattr(self, done_flag_name, True)
            return
        ...
        if success:
            self.node.get_logger().info("Fixed-pose Nav2 goal succeeded.")
        else:
            self.node.get_logger().error(f"Fixed-pose Nav2 goal failed with status: {status}")

        setattr(self, success_flag_name, success)
        setattr(self, done_flag_name, True)
